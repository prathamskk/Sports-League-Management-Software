...
import com.project.sportsleaguemanagementproject.singleton.PlayerIDSingleton;
...
public class TeamViewPlayerStatsController implements Initializable {
    private Connection con;
    @FXML
    private VBox mainVbox;
    private String verification_status;
    private int playerID1 =PlayerIDSingleton.getInstance().id;
    ...
    private boolean checkAlreadyRegistered() throws SQLException {
        ResultSet rs = con.createStatement().executeQuery("select verification_status from player where player_id = '"+playerID1+"'");
        if(rs.next()){
    ...
    private void displayInfo() throws SQLException {
        if(checkAlreadyRegistered()){
            if(verification_status.equals("verified")){
                ...
                ResultSet rs = con.createStatement().executeQuery("select player_id from player where player_id='"+playerID1+"'");
                rs.next();
...
//THIS SHOULD NEVER RUN IN THIS CONTROLLER
            }

        }else{
            //FOR FIRST TIME USERS (UNREGISTERED PLAYERS) //THIS SHOULD NEVER RUN IN THIS CONTROLLER
